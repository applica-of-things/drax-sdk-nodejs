class Uint8Matrix {
	constructor(sizeRow, sizeCol){
		this.array = new Uint8Array(sizeRow * sizeCol)
	...
	get(x, y){
		return this.array[x + y * this.sizeRow]
	}

	set(x, y, value){
		this.array[x + y * this.sizeRow] = value
	}
}
...
function KE_ROTWORD(x) {
	var _t = new Uint32Array(1)
	_t[0] = x
  	var _r = new Uint32Array(2) 
	_r[0] = _t[0] << 8
	_r[1] = _t[0] >> 24 & 0xFF
	return _r[0] | _r[1]
}
...
function SubWord(word) {
	...
	result[0] = parseInt(aes_sbox[(word >> 4) & 0x0000000F][word & 0x0000000F]);
	result[0] += parseInt(aes_sbox[(word >> 12) & 0x0000000F][(word >> 8) & 0x0000000F] << 8);
	result[0] += parseInt(aes_sbox[(word >> 20) & 0x0000000F][(word >> 16) & 0x0000000F] << 16);
	result[0] += parseInt(aes_sbox[(word >> 28) & 0x0000000F][(word >> 24) & 0x0000000F] << 24);
	return(result[0]);
}

function AddRoundKey(state, w, pos) {
	var subkey = new Uint8Array(4);
...
{
	state.set(0, 0, aes_sbox[state.get(0, 0) >> 4][state.get(0, 0) & 0x0F])
	state.set(0, 1, aes_sbox[state.get(0, 1) >> 4][state.get(0, 1) & 0x0F])
	state.set(0, 2, aes_sbox[state.get(0, 2) >> 4][state.get(0, 2) & 0x0F])
	state.set(0, 3, aes_sbox[state.get(0, 3) >> 4][state.get(0, 3) & 0x0F])
	state.set(1, 0, aes_sbox[state.get(1, 0) >> 4][state.get(1, 0) & 0x0F])
	state.set(1, 1, aes_sbox[state.get(1, 1) >> 4][state.get(1, 1) & 0x0F])
	state.set(1, 2, aes_sbox[state.get(1, 2) >> 4][state.get(1, 2) & 0x0F])
	state.set(1, 3, aes_sbox[state.get(1, 3) >> 4][state.get(1, 3) & 0x0F])
	state.set(2, 0, aes_sbox[state.get(2, 0) >> 4][state.get(2, 0) & 0x0F])
	state.set(2, 1, aes_sbox[state.get(2, 1) >> 4][state.get(2, 1) & 0x0F])
	state.set(2, 2, aes_sbox[state.get(2, 2) >> 4][state.get(2, 2) & 0x0F])
	state.set(2, 3, aes_sbox[state.get(2, 3) >> 4][state.get(2, 3) & 0x0F])
	state.set(3, 0, aes_sbox[state.get(3, 0) >> 4][state.get(3, 0) & 0x0F])
	state.set(3, 1, aes_sbox[state.get(3, 1) >> 4][state.get(3, 1) & 0x0F])
	state.set(3, 2, aes_sbox[state.get(3, 2) >> 4][state.get(3, 2) & 0x0F])
	state.set(3, 3, aes_sbox[state.get(3, 3) >> 4][state.get(3, 3) & 0x0F])
}

function InvSubBytes(state)
{
	state.set(0, 0, aes_invsbox[state.get(0, 0) >> 4][state.get(0, 0) & 0x0F])
	state.set(0, 1, aes_invsbox[state.get(0, 1) >> 4][state.get(0, 1) & 0x0F])
	state.set(0, 2, aes_invsbox[state.get(0, 2) >> 4][state.get(0, 2) & 0x0F])
	state.set(0, 3, aes_invsbox[state.get(0, 3) >> 4][state.get(0, 3) & 0x0F])
	state.set(1, 0, aes_invsbox[state.get(1, 0) >> 4][state.get(1, 0) & 0x0F])
	state.set(1, 1, aes_invsbox[state.get(1, 1) >> 4][state.get(1, 1) & 0x0F])
	state.set(1, 2, aes_invsbox[state.get(1, 2) >> 4][state.get(1, 2) & 0x0F])
	state.set(1, 3, aes_invsbox[state.get(1, 3) >> 4][state.get(1, 3) & 0x0F])
	state.set(2, 0, aes_invsbox[state.get(2, 0) >> 4][state.get(2, 0) & 0x0F])
	state.set(2, 1, aes_invsbox[state.get(2, 1) >> 4][state.get(2, 1) & 0x0F])
	state.set(2, 2, aes_invsbox[state.get(2, 2) >> 4][state.get(2, 2) & 0x0F])
	state.set(2, 3, aes_invsbox[state.get(2, 3) >> 4][state.get(2, 3) & 0x0F])
	state.set(3, 0, aes_invsbox[state.get(3, 0) >> 4][state.get(3, 0) & 0x0F])
	state.set(3, 1, aes_invsbox[state.get(3, 1) >> 4][state.get(3, 1) & 0x0F])
	state.set(3, 2, aes_invsbox[state.get(3, 2) >> 4][state.get(3, 2) & 0x0F])
	state.set(3, 3, aes_invsbox[state.get(3, 3) >> 4][state.get(3, 3) & 0x0F])
}

function aes_key_setup(key, w, keysize){
  var Nb = 4, Nr, Nk, idx
  var Rcon = new Uint32Array([0x01000000,0x02000000,0x04000000,0x08000000,0x10000000,0x20000000,
    ...
    for (idx = Nk; idx < Nb * (Nr+1); ++idx) {
      temp[0] = w[idx - 1];
      if ((idx % Nk) == 0)
        temp[0] = SubWord(KE_ROTWORD(temp[0])) ^ Rcon[Math.floor((idx - 1) / Nk)];
...
function aes_encrypt(input, pos, out,  key, keysize){
  	var state = new Uint8Matrix(4, 4)
...
function aes_decrypt(input, pos, out, key, keysize)
{
	var state = new Uint8Matrix(4, 4)
